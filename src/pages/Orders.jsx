  ...
  const { data: orders = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [purchases, sales] = await Promise.all([
        OrderService.filter({ buyer_email: user.email }, "-created_date", 50),
        OrderService.filter({ seller_email: user.email }, "-created_date", 50),
      ]);
      return Array.from(new Map([...purchases, ...sales].map((order) => [order.id, order])).values())
