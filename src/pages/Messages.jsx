...
import { Mail, User, MessageSquare, Clock, CheckCircle, DollarSign, ArrowLeft } from "lucide-react";
...
export default function Messages() {
  ...
  const { data: messages, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const allMsgs = await MessageService.filter({}, "-createdAt", 100);
       // Filter messages where user is sender or recipient
       return (allMsgs || []).filter(
         msg => msg.sender_email === user.email || msg.recipient_email === user.email
       );
      },
    ...
    messages.forEach(msg => {
      if (!grouped[msg.conversation_id]) {
        grouped[msg.conversation_id] = {
          ...
          other_user_email: msg.sender_email === (messages[0]?.sender_email) ? msg.recipient_email : msg.sender_email,
      ...
      if (msg.recipient_email === messages[0]?.sender_email && !msg.is_read) {
  ...
  const filteredConversations = conversations.filter(conv => {
    ...
    if (filter === "sent") return matchesSearch && conv.last_message.sender_email === messages?.[0]?.sender_email;
    if (filter === "received") return matchesSearch && conv.last_message.sender_email !== messages?.[0]?.sender_email;
