...
export default function ArtistVerifications() {
  ...
  const { data: verifications = [], isLoading } = useQuery({
    ...
    queryFn: () => UserVerificationService.filter({}, "-created_date", 50),
    initialData: [],
  });

  const approveMutation = useMutation({
    mutationFn: async ({ verificationId }) => {
      const response = await httpsCallable(functions, 'approveVerification')({ verificationId, action: 'approve' });
    ...
    onError: (error) => toast.error(error.message || "Failed to approve verification"),
  });

  const rejectMutation = useMutation({
    mutationFn: async ({ verificationId, reason }) => {
      const response = await httpsCallable(functions, 'approveVerification')({ verificationId, action: 'reject', notes: reason });
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["artist-verifications"] });
    ...
    onError: (error) => toast.error(error.message || "Failed to reject verification"),
  });
