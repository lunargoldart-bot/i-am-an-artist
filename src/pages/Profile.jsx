...
import { User, Palette, Instagram, Globe, Edit3, Diamond, Megaphone, Crown, Users, Plus } from "lucide-react";
...
export default function Profile() {
  ...
  const { data: myArtworks = [], refetch: refetchArtworks } = useQuery({
    ...
    queryFn: async () => {
      ...
      return ArtworkService.filter({ artist_email: user.email }, "-createdAt", 50);
    },
    enabled: !!user?.email,
    initialData: [],
  });
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      {collaborationFormOpen && (
        <CollaborationRequestForm
          onClose={() => setCollaborationFormOpen(false)}
          onSuccess={() => queryClient.invalidateQueries({ queryKey: ['collaboration-requests'] })}
        />
      )}
    </div>
  );
