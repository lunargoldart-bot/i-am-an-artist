...
import { Upload, ImagePlus, Sparkles, ShoppingBag, Gavel } from "lucide-react";
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="space-y-2">
          <Label className="font-body">Artwork Image</Label>
          <label className="flex flex-col items-center justify-center w-full aspect-[4/3] rounded-xl border-2 border-dashed border-border hover:border-primary/50 cursor-pointer transition-colors bg-muted/30 overflow-hidden">
            {previewUrl ? (
              ...
              <div className="flex flex-col items-center gap-2 text-muted-foreground">
                ...
                <span className="font-body text-sm">Click to upload image</span>
              </div>
            )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2 sm:col-span-2">
            ...
            <Input value={form.title} onChange={(e) => setForm({...form, title: e.target.value})} placeholder="Name of your artwork" className="font-body" required />
          </div>

          <div className="space-y-2">
            <Label className="font-body">Artist Name *</Label>
            <Input value={form.artist_name} onChange={(e) => setForm({...form, artist_name: e.target.value})} placeholder="Your artist name" className="font-body" required />
          </div>
          ...
          <div className="space-y-2">
            <Label className="font-body">Medium</Label>
            <Input value={form.medium} onChange={(e) => setForm({...form, medium: e.target.value})} placeholder="Oil, acrylic, digital..." className="font-body" />
          </div>

          <div className="space-y-2">
            <Label className="font-body">Dimensions</Label>
            <Input value={form.dimensions} onChange={(e) => setForm({...form, dimensions: e.target.value})} placeholder="e.g. 60cm x 80cm" className="font-body" />
          </div>
        ...
        <div className="space-y-2">
          ...
          <Textarea value={form.description} onChange={(e) => setForm({...form, description: e.target.value})} placeholder="Tell the story behind this piece..." className="font-body" rows={4} />
        </div>
        ...
        {form.is_auction && (
          <div className="space-y-2">
            <Label className="font-body">Auction Duration</Label>
            <Select value={form.auction_duration} onValueChange={(v) => setForm({...form, auction_duration: v})}>
              <SelectTrigger className="font-body"><SelectValue placeholder="Select duration" /></SelectTrigger>
              <SelectContent>
        ...
        <Button type="submit" size="lg" disabled={createMutation.isPending || uploading} className="w-full rounded-full font-body">
          {uploading ? "Uploading..." : createMutation.isPending ? "Listing..." : (form.is_auction ? "Start Auction" : "List Artwork for Sale")}
        </Button>
      </form>
    </div>
  );
