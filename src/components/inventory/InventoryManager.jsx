...
export default function InventoryManager() {
  ...
  const { data: artworks = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return firebaseClient.entities.Artwork.filter({ artist_email: user.email }, '-created_date', 100);
    },
    enabled: !!user?.email
  });
  ...
  const updateStatusMutation = useMutation({
    mutationFn: ({ artworkId, newStatus }) => 
      firebaseClient.entities.Artwork.update(artworkId, { status: newStatus }),
    onSuccess: () => {
