...
export default function ArtistCard({ artist, rank }) {
  ...
  return (
    <Link to={`/artist/${artist.id}`} className="gallery-card-hover bg-card border border-border rounded-lg overflow-hidden group p-4 flex items-center gap-4 cursor-pointer">
      {rank && (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 mb-0.5">
          ...
          {artist.is_verified && <CheckCircle className="w-3.5 h-3.5 text-gold shrink-0" />}
        </div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <span className="flex items-center gap-0.5 text-xs text-muted-foreground">
            <Users className="w-3 h-3" /> {(artist.followers_count || 0).toLocaleString()}
          </span>
          <span className="flex items-center gap-0.5 text-xs text-muted-foreground">
            <ShoppingBag className="w-3 h-3" /> {(artist.total_sales || 0).toLocaleString()} sales
          </span>
        </div>
      </div>
