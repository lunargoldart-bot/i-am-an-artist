...
export default function WishlistButton({ artwork, variant = "icon", className }) {
  ...
  const { data: wishlistItems = [] } = useQuery({
    ...
    queryFn: () => firebaseClient.entities.Wishlist.filter({ user_email: user.email }),
    enabled: !!user?.email,
  });
  ...
  const addMutation = useMutation({
    mutationFn: () =>
      firebaseClient.entities.Wishlist.create({
  ...
  const removeMutation = useMutation({
    mutationFn: () => firebaseClient.entities.Wishlist.delete(saved.id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["wishlist"] });
