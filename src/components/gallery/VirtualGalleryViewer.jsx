...
import { ChevronLeft, ChevronRight, ShoppingCart, Maximize2, X } from 'lucide-react';
...
export default function VirtualGalleryViewer({ exhibitionId }) {
  ...
  const { data: exhibition = {} } = useQuery({
    ...
    queryFn: () => base44.entities.Exhibition.get(exhibitionId)
  });
  ...
  const { data: artworks = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const works = await Promise.all(
        exhibition.artwork_ids.map(id => base44.entities.Artwork.get(id))
      );
