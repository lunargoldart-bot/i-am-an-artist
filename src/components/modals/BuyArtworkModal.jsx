...
import { X, Shield, CheckCircle, Loader2, Smartphone } from 'lucide-react';
...
export default function BuyArtworkModal({ artwork, onClose }) {
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      const response = await firebaseClient.functions.invoke('createOrder', {
  ...
  const handleInitiatePayment = async () => {
    ...
    try {
      ...
      const response = await firebaseClient.functions.invoke('initiateEscrowPayment', {
        orderId,
        artworkId: artwork.id,
        phone,
