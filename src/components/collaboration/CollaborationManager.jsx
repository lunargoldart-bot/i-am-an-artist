...
export default function CollaborationManager() {
  ...
  const { data: requests = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [sent, received] = await Promise.all([
        firebaseClient.entities.CollaborationRequest.filter({ initiator_email: user.email }, '-created_date', 50),
        firebaseClient.entities.CollaborationRequest.filter({ collaborator_email: user.email }, '-created_date', 50),
      ]);
      return Array.from(new Map([...sent, ...received].map((item) => [item.id, item])).values());
    },
    initialData: [],
    enabled: !!user
  });
  ...
  const { data: collaborations = [] } = useQuery({
    ...
    queryFn: async () => {
      const [initiated, joined] = await Promise.all([
        firebaseClient.entities.Collaboration.filter({ initiator_email: user.email }, '-created_date', 50),
        firebaseClient.entities.Collaboration.filter({ collaborator_email: user.email }, '-created_date', 50),
      ]);
      return Array.from(new Map([...initiated, ...joined].map((item) => [item.id, item])).values());
    },
    initialData: [],
    enabled: !!user
  });
  ...
  const handleMutation = useMutation({
    mutationFn: ({ requestId, action }) =>
      firebaseClient.functions.invoke('handleCollaborationRequest', { requestId, action }),
