...
export default function ManageAds() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [myAds, myArtworks] = await Promise.all([
        base44.entities.SponsoredAd.filter({ artist_email: me.email }, '-created_date'),
        base44.entities.Artwork.filter({ artist_email: me.email }),
      ]);
      setAds(myAds || []);
  ...
  const handleCreate = async () => {
    ...
    setSubmitting(true);
    await base44.entities.SponsoredAd.create({
      ...form,
    ...
    toast.success('Ad launched!');
    setShowForm(false);
    setForm({ headline: '', tagline: '', image_url: '', cta_label: 'View Artwork', cta_link: '', start_date: '', end_date: '' });
    const updated = await base44.entities.SponsoredAd.filter({ artist_email: user.email }, '-created_date');
    setAds(updated || []);
    setSubmitting(false);
  };
  ...
  const toggleStatus = async (ad) => {
    ...
    await base44.entities.SponsoredAd.update(ad.id, { status: next });
    setAds(prev => prev.map(a => a.id === ad.id ? { ...a, status: next } : a));
  };

  const deleteAd = async (id) => {
    await base44.entities.SponsoredAd.delete(id);
