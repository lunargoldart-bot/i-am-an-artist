...
export default function LipilaPaymentModal({ open, onClose, ad, onSuccess }) {
  ...
  const handlePay = async () => {
    ...
    // Update ad with end date before payment
    await base44.entities.SponsoredAd.update(ad.id, {
      end_date: endDate.toISOString().split('T')[0],
      status: 'paused', // will be activated on successful callback
    });

    const res = await base44.functions.invoke('lipila_initiate', {
      amount: selectedPlan.amount,
      phone,
      narration: `Zartia Sponsored Ad — ${ad.headline}`,
      referenceId,
      adId: ad.id,
    });
