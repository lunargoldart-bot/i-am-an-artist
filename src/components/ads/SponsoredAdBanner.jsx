...
export default function SponsoredAdBanner({ placement = 'feed' }) {
  ...
  useEffect(() => {
    base44.entities.SponsoredAd.filter({ status: 'active' }, '-created_date', 20)
      .then(ads => {
        ...
        // Track impression
        if (pick?.id) {
          base44.entities.SponsoredAd.update(pick.id, { impressions: (pick.impressions || 0) + 1 }).catch(() => {});
        }
      }).catch(() => {});
  }, []);
  ...
  const handleClick = () => {
    if (ad?.id) {
      base44.entities.SponsoredAd.update(ad.id, { clicks: (ad.clicks || 0) + 1 }).catch(() => {});
    }
  };
