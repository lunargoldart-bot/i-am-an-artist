...
import { User, CheckCircle, DollarSign, MessageSquare } from "lucide-react";
...
export default function MessageThread({ artworkId, recipientEmail, currentUserEmail }) {
  ...
  const { data: messages, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [sent, received] = await Promise.all([
        firebaseClient.entities.Message.filter(
          { conversation_id: conversationId, sender_email: currentUserEmail },
          "created_date",
          50
        ),
        firebaseClient.entities.Message.filter(
          { conversation_id: conversationId, recipient_email: currentUserEmail },
          "created_date",
          50
        ),
      ]);
    ...
    enabled: Boolean(conversationId && currentUserEmail),
  });
  ...
  const markReadMutation = useMutation({
    mutationFn: async () => {
      if (conversationId) {
        await firebaseClient.functions.invoke('markMessagesRead', { conversation_id: conversationId });
      }
    },
    onSuccess: () => {
  ...
  useEffect(() => {
    ...
    const refresh = () => queryClient.invalidateQueries({ queryKey: ["messages", conversationId] });
    const unsubscribeSent = firebaseClient.entities.Message.subscribe(refresh, {
      where: { conversation_id: conversationId, sender_email: currentUserEmail },
      orderBy: 'created_date',
      limit: 50,
    });
    const unsubscribeReceived = firebaseClient.entities.Message.subscribe(refresh, {
      where: { conversation_id: conversationId, recipient_email: currentUserEmail },
      orderBy: 'created_date',
      limit: 50,
    });
