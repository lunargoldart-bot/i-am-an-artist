...
export default function ManageExhibitions({ user }) {
  ...
  const load = () => {
    if (!user) return;
    base44.entities.Exhibition.filter({ organizer_email: user.email }).then(setExhibitions).catch(() => {});
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (editing) {
      await base44.entities.Exhibition.update(editing.id, data);
      toast.success('Exhibition updated!');
    } else {
      await base44.entities.Exhibition.create(data);
  ...
  const handleDelete = async (id) => {
    ...
    await base44.entities.Exhibition.delete(id);
