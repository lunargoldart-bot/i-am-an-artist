...
export default function MyOrders({ user }) {
  ...
  useEffect(() => {
    if (!user) return;
    firebaseClient.entities.Order.filter({ buyer_email: user.email }).then(setPurchases).catch(() => {});
    firebaseClient.entities.Order.filter({ seller_email: user.email }).then(setSales).catch(() => {});
  }, [user]);

  const markDelivered = async (order) => {
    await firebaseClient.functions.invoke('confirmDelivery', { orderId: order.id });
    toast.success('Delivery confirmed. Payment release will follow the escrow rules.');
    firebaseClient.entities.Order.filter({ buyer_email: user.email }).then(setPurchases);
  };
