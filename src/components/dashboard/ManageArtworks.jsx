...
export default function ManageArtworks({ user }) {
  ...
  const load = () => {
    if (!user) return;
    base44.entities.Artwork.filter({ artist_email: user.email }).then(setArtworks).catch(() => {});
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (editing) {
      await base44.entities.Artwork.update(editing.id, data);
      toast.success('Artwork updated!');
    } else {
      await base44.entities.Artwork.create(data);
  ...
  const handleDelete = async (id) => {
    ...
    await base44.entities.Artwork.delete(id);
