...
export default function ManageNews({ user }) {
  ...
  const load = () => {
    if (!user) return;
    firebaseClient.entities.NewsFeed.filter({ author_email: user.email }).then(setNews).catch(() => {});
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (editing) {
      await firebaseClient.entities.NewsFeed.update(editing.id, data);
      toast.success('Post updated!');
    } else {
      await firebaseClient.entities.NewsFeed.create(data);
  ...
  const handleDelete = async (id) => {
    ...
    await firebaseClient.entities.NewsFeed.delete(id);
