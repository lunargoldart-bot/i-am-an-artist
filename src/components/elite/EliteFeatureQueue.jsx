...
import { Diamond, Calendar, Plus, Trash2, Crown, ChevronRight } from 'lucide-react';
...
export default function EliteFeatureQueue() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [artworks, existing] = await Promise.all([
        ...
        base44.entities.FeatureQueue.filter({ artist_email: me.email }),
      ]);
  ...
  const handleQueue = async () => {
    ...
    setSubmitting(true);
    const art = myArtworks.find(a => a.id === selectedArtwork);
    await base44.entities.FeatureQueue.create({
    ...
    setSelectedDate('');
    // Refresh queue
    const updated = await base44.entities.FeatureQueue.filter({ artist_email: user.email });
    setQueue((updated || []).sort((a, b) => new Date(a.feature_date) - new Date(b.feature_date)));
    setSubmitting(false);
  };

  const handleCancel = async (id) => {
    await base44.entities.FeatureQueue.update(id, { status: 'cancelled' });
